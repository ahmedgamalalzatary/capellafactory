...
import { getPurchaseCorrection } from "@/lib/api/purchase-corrections";
...
type PurchaseCorrectionDetailPageProps = {
  params: Promise<{
    id: string;
  }>;
};

export default async function PurchaseCorrectionDetailPage({
  params,
}: PurchaseCorrectionDetailPageProps) {
  ...
  const [correction, ingredients] = await Promise.all([
    getPurchaseCorrection(correctionId, { cookieHeader }).catch(() => null),
  ...
  const total = correction.lines.reduce(
    (sum: number, line: PurchaseCorrectionLine) => sum + line.lineTotal,
    0,
  );
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-6 sm:px-8 sm:py-8">
      <div className="mb-5">
        <Link
          href="/purchases?tab=purchase-corrections"
        ...
        <div className="border-b px-5 py-6 sm:px-8">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="grid grid-cols-2 gap-3 sm:min-w-90">
              <Metric label="عدد البنود" value={String(correction.lines.length)} />
              <Metric label="إجمالي العكس" value={formatAmount(total)} />
            </div>
          </div>
        </div>

        <div className="grid gap-4 px-5 py-5 sm:grid-cols-3 sm:px-8">
          <DetailItem label="الفاتورة الأصلية" value={correction.sourcePurchaseInvoiceCode ?? `#${correction.sourcePurchaseId}`} />
          <DetailItem label="وقت التسجيل" value={formatDateTime(correction.createdAt)} />
          <DetailItem label="السبب" value={correction.reason} />
        </div>

        <div className="border-t px-5 py-5 sm:px-8">
          <h2 className="mb-4 text-[17px] font-bold text-slate-950">بنود العكس</h2>
          <div className="hidden sm:block overflow-x-auto rounded-2xl border">
            <Table>
              ...
              <TableBody>
                {correction.lines.map((line: PurchaseCorrectionLine) => (
          ...
          <div className="divide-y sm:hidden overflow-hidden rounded-2xl border">
            {correction.lines.map((line: PurchaseCorrectionLine, idx: number) => (
              <PurchaseCorrectionLineCard
                key={line.id}
                line={line}
...
function PurchaseCorrectionLineCard({
  line,
  ingredientName,
  idx,
}: {
